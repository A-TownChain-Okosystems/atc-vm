...
impl Parser {
    ...
    fn expr(&mut self) -> Result<Vec<String>, String> {
        ...
        loop {
            let op = match self.peek() { Some("+") => "+", Some("-") => "-", _ => break };
            self.pos += 1;
    ...
    fn factor(&mut self) -> Result<Vec<String>, String> {
        ...
        if tok == "(" {
            let ops = self.expr()?;
            if self.take()? != ")" { return Err("unbalanced parenthesis".to_string()); }
            Ok(ops)
...
fn tokenize(source: &str) -> Result<Vec<String>, String> {
    ...
    while i < chars.len() {
        ...
        if matches!(c, '+' | '-' | '*' | '/' | '(' | ')') {
            out.push(c.to_string()); i += 1; continue;
        }
...
fn find_function(source: &str) -> Result<(Vec<String>, String), String> {
    ...
    let open = after_name.find('(').ok_or_else(|| "function parameter list missing".to_string())?;
    let params_start = fn_pos + 3 + open + 1;
    let params_end = source[params_start..].find(')').map(|p| params_start + p)
        .ok_or_else(|| "function parameter list is unclosed".to_string())?;
    let params = source[params_start..params_end].split(',')
        .filter_map(|p| p.trim().split(':').next())
        .map(str::trim).filter(|p| !p.is_empty()).map(ToOwned::to_owned).collect::<Vec<_>>();
    let body_open = source[params_end..].find('{').map(|p| params_end + p)
        .ok_or_else(|| "function body missing".to_string())?;
    let body_end = source[body_open + 1..].find('}').map(|p| body_open + 1 + p)
...
fn json_u64_for_key(json: &str, key: &str) -> Result<u64, String> {
    ...
    if end == 0 { return Err(format!("vector value for {key} is not an unsigned integer")); }
    value[..end].parse::<u64>().map_err(|_| format!("vector value for {key} is not u64"))
}
...
pub fn assemble(contract: &Path, vector: &Path, out_dir: &Path) -> Result<PathBuf, String> {
    ...
    for line in body.lines() {
        ...
        if let Some(rest) = line.strip_prefix("let ") {
            ...
            let ops = parser.expr()?;
            if parser.peek().is_some() { return Err(format!("unconsumed expression tokens in: {line}")); }
            ...
            let ops = parser.expr()?;
            if parser.peek().is_some() { return Err(format!("unconsumed return expression tokens in: {line}")); }
            final_ops = Some(ops);
        }
    }
